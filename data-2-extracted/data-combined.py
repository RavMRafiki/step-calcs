...
# Time window (seconds) to show on the chart
PLOT_START_SEC = 50
PLOT_END_SEC = 200
...
combined_df = combined_df[(combined_df['time_sec'] >= PLOT_START_SEC) & (combined_df['time_sec'] <= PLOT_END_SEC)].reset_index(drop=True)
combined_df.to_csv('combined_heel_strikes.csv', index=False)

# 5. Visualize and save the chart
df_acc1_plot = df_acc1[(df_acc1['time_sec'] >= PLOT_START_SEC) & (df_acc1['time_sec'] <= PLOT_END_SEC)]
df_acc2_plot = df_acc2[(df_acc2['time_sec'] >= PLOT_START_SEC) & (df_acc2['time_sec'] <= PLOT_END_SEC)]
df_peaks1_plot = df_peaks1[(df_peaks1['time_sec'] >= PLOT_START_SEC) & (df_peaks1['time_sec'] <= PLOT_END_SEC)]
df_peaks2_plot = df_peaks2[(df_peaks2['time_sec'] >= PLOT_START_SEC) & (df_peaks2['time_sec'] <= PLOT_END_SEC)]

# Using the ultra-wide figsize requested
plt.figure(figsize=(50, 6))

# Plot the raw signals
plt.plot(df_acc1_plot['time_sec'], df_acc1_plot['x'], label='Accelerometer X-axis (215226)', color='pink', linewidth=1)
plt.plot(df_acc2_plot['time_sec'], df_acc2_plot['x'], label='Accelerometer X-axis (215231)', color='lightblue', linewidth=1, alpha=0.7)

# Plot the detected ground contacts (heel strikes)
plt.plot(df_peaks1_plot['time_sec'], df_peaks1_plot['x'], "ro", label='Heel Strikes (215226)')
plt.plot(df_peaks2_plot['time_sec'], df_peaks2_plot['x'], "bo", label='Heel Strikes (215231)')

plt.title("Combined Walking Gait Analysis: Ground Contact Detection")
plt.xlabel("Time (seconds)")
plt.ylabel("Acceleration")
plt.xlim(PLOT_START_SEC, PLOT_END_SEC)
plt.legend()
